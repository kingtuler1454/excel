...
def read_cicle(filename: str, Solder:bool): # 44 столбца заранее известных
    ...
    for student_number in trange (5,sheet.max_row + 1):
        if sheet.cell(row=student_number, column=1).value==None:break
    ...
    for i in range (len(list1)-1):
        for j in range (i+1,len(list1)-2):
            if list1[i].get("Дата рождения")==list1[j].get("Дата рождения"):
                print(list1[i].get("Дата рождения"))
                print(list1[j].get("Дата рождения"))
